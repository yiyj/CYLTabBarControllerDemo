...
typealias AlterTextViewTextDidChangeClouse = (CGFloat) -> ()

class AlterTextView: UITextView {
    ...
    var textChangeClouse: AlterTextViewTextDidChangeClouse?
    ...
    func setupTextView(){
        ...
        self.maxLength = AlterlManager.share.maxLength
        self.onlyNumberAndPoint = AlterlManager.share.onlyNumberAndPoint
        self.onlyNumber = AlterlManager.share.onlyNumber
        self.allowEmoji = AlterlManager.share.allowEmoji
        self.pointLength = AlterlManager.share.pointLength
        self.predicateString = AlterlManager.share.predicateString
    }
    //MARK - 监听文字改变
    @objc func textDidChange(notic: Notification){
        ...
        if self.maxLength != nil && self.maxLength! > 0 {
            if (self.text.count) > (self.maxLength)! {
                let subString = self.text.subToString(to: (self.maxLength)!)
                self.text = subString
            }
        }
    ...
    func dealPredicate() {
        ...
        var newInputStr = self.text.subFromString(from: self.tempText?.count ?? 0)
        ...
        if self.pointLength != nil && self.pointLength! > 0 {
            ...
            if arr?.count == 2 { // 有小数点
                ...
                if pointConut >= self.pointLength! {
                    self.tempText = (arr?.first ?? "") + "." + (arr?.last?.subToString(to: self.pointLength!) ?? "")
                    newInputStr = ""
                } else {
                    let count = (arr?.last?.count ?? 0) + newInputStr.count
                    if count > self.pointLength! {
                        let newCount = count - self.pointLength!
                        newInputStr = newInputStr.subToString(to: newCount)
                    }
                }
            }
        }
