...
typealias AlterInputViewClouse = (String)->()

class AlterInputView: UIView {
    ...
    lazy var textView: AlterTextView = {
        let text = AlterTextView.init(frame: CGRect.zero)
        text.placehoder = "请输入..."
    ...
    open var leftHandler: AlterInputViewClouse?
    open var rightHandler: AlterInputViewClouse?
    ...
    @objc func clickLeftbtn() {
        AlterlManager.share.id_resetInputProps()
    ...
    override func layoutSubviews() {
        ...
        if self.leftActionTitle == nil || self.leftActionTitle?.count == 0 {
            ...
            self.rightBtn.setTitleColor(AlterlManager.share.mainColor, for: .normal)
            return
        }
        if self.rightActionTitle == nil || self.rightActionTitle?.count == 0 {
            ...
            self.frame = CGRect.init(x: 0.5*(superWidth - dialogWidth), y: 0.5*(superHeight - (self.bottomView.frame.maxY)), width: dialogWidth, height: self.bottomView.frame.maxY)
            self.leftBtn.setTitleColor(AlterlManager.share.mainColor, for: .normal)
            return
        }
